...
export async function sendStartMessage(chatId) {
  try {
    ...
    await bot.sendMessage(chatId, 'Что вы хотите сделать?', options);
  }
  catch (error) {
    log.error(error);
  }
...
export async function createNewCategory(chatId, userId) {
  try {
    ...
    await bot.sendMessage(chatId, text, null, false, callback);
  }
  catch (error) {
    log.error(error);
  }
...
export async function showCategoryNotes(chatId, userId, categoryName) {
  ...
  if (notes.length) {
    notes.forEach(async note => await bot.sendMessage(chatId, note));
  } else {
    await bot.sendMessage(chatId, 'Записей не найдено');
  }
}
