    ...
    const options = {
      reply_markup: {
        inline_keyboard: [
          [
            {
              text: 'Посмотреть существующие записи',
              callback_data: START_MESSAGE_CHOSE_CATEGORY
            }
          ],
          [
            {
              text: 'Добавить новую категорию',
              callback_data: START_MESSAGE_CREATE_CATEGORY
            }
          ],
          [
            {
              text: 'Посмотреть последние 20 сохраненных записей',
              callback_data: START_MESSAGE_SHOW_LAST_SAVED_NOTES
            }
          ]
        ]
      }
...
export async function createNewCategory(chatId, userId) {
  ...
  async function callback(categoryName) {
    try {
      const isCategoryAlreadyCreated = await UserCategoriesService.addNewCategory(
        userId,
        categoryName
      )
      ...
      if (isCategoryAlreadyCreated) {
        ...
        bot.sendMessage({
          chatId,
          text: `Категория "${categoryName}" добавлена`
        })
      }
...
export async function showNotes(chatId, userId, categoryName) {
  ...
  if (notes.length) {
    await Promise.all(
      notes.map(note =>
        bot.sendMessage({ chatId, text: note, isSavedState: true })
      )
    )
  } else {
