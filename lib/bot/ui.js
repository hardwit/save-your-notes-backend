...
export async function sendStartMessage(chatId) {
  ...
  helper.setState(chatId, message.message_id, 'waitingForChoice', sendStartMessage.bind(null, chatId));
}
...
export async function createNewCategory(chatId, userId) {
  ...
  helper.setState(chatId, messageId, 'waitingForInput', callback);
}
...
export async function displayCategories(chatId) {
  const { messageId } = helper.getChatState(chatId);
